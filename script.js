...
const wordlistList = document.getElementById('wordlistList');
...
function initializeInterface() {
    ...
    savedWorkflows.forEach(workflow => {
        ...
        item.onclick = () => selectWorkflow(workflow);
        workflowList.appendChild(item);
    });

    // Add "New Workflow" option
    const newWorkflowItem = document.createElement('div');
    newWorkflowItem.className = 'selection-item';
    newWorkflowItem.textContent = 'New Workflow';
    newWorkflowItem.onclick = () => {
        closeModal(workflowModal);
        showCreateWorkflow();
    };
    workflowList.appendChild(newWorkflowItem);

    // Populate wordlist list
    wordlistList.innerHTML = '';
    const wordlists = ['ENUK Wordlist']; // Add more wordlists as needed
    wordlists.forEach(wordlist => {
        const item = document.createElement('div');
        item.className = 'selection-item';
        item.textContent = wordlist;
        item.onclick = () => selectWordlist(wordlist);
        wordlistList.appendChild(item);
    });

    // Set up event listeners
    workflowButton.onclick = () => showModal(workflowModal);
    wordlistButton.onclick = () => showModal(wordlistModal);
    createWorkflowButton.onclick = showCreateWorkflow;
    performButton.onclick = performWorkflow;

    // Close modals when clicking outside
    window.onclick = (event) => {
        if (event.target === workflowModal) closeModal(workflowModal);
        if (event.target === wordlistModal) closeModal(wordlistModal);
    };
...
function selectWorkflow(workflow) {
    ...
    workflowButton.querySelector('.selected-text').textContent = workflow.name;
    closeModal(workflowModal);
    updatePerformButton();
}
...
function selectWordlist(wordlist) {
    ...
    wordlistButton.querySelector('.selected-text').textContent = wordlist;
    closeModal(wordlistModal);
    updatePerformButton();
}

function updatePerformButton() {
    performButton.disabled = !selectedWorkflow || !selectedWordlist;
}

function showCreateWorkflow() {
    // Hide homepage and show workflow creation interface
    document.getElementById('homepage').style.display = 'none';
    document.getElementById('workflowCreation').style.display = 'block';
    // Reset workflow creation form
    resetWorkflowForm();
}

function performWorkflow() {
    if (!selectedWorkflow || !selectedWordlist) return;
