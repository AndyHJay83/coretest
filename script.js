...
function initializeWorkflowDropdown() {
    ...
    `;
    workflowDropdown.appendChild(customSelect);
    ...
    const allOptions = [...defaultOptions, ...workflowOptions];
    ...
    selectedText.textContent = 'Select a workflow...';
...
function initializeWordlistDropdown() {
    const wordlistDropdown = document.querySelectorAll('.dropdown')[1];
    if (!wordlistDropdown) {
        console.error('Wordlist dropdown container not found');
        return;
    }
    
    // Remove existing custom select if it exists
    const existingCustomSelect = wordlistDropdown.querySelector('.custom-select');
    if (existingCustomSelect) {
        existingCustomSelect.remove();
    }
    ...
    customSelect.innerHTML = `
        ...
        <div class="options-list"></div>
    `;
    wordlistDropdown.appendChild(customSelect);
    ...
    // Set initial selected text
    selectedText.textContent = 'ENUK Wordlist';
    ...
    customSelect.addEventListener('click', (e) => {
        ...
        if (!isVisible) {
            // Get the position of the custom select relative to the viewport
            const customSelectRect = customSelect.getBoundingClientRect();
            const dropdownRect = wordlistDropdown.getBoundingClientRect();
    ...
    // Store reference to custom select for cleanup
    wordlistDropdown._customSelect = customSelect;
}
...
function hideWorkflowCreation() {
    ...
    // Ensure dropdowns are initialized when returning to homepage
    initializeWorkflowDropdown();
    initializeDropdowns();
}
